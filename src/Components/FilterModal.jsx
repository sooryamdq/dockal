  ...
  const applyFilter = () => {
    ...
    switch (filterType) {
      case "year":
        ...
        break;
      case "dept":
        results = results.filter((item) => item.dept.toLowerCase().includes(filterValue.toLowerCase()));
        break;
      case "section":
        results = results.filter((item) => item.section.toLowerCase().includes(filterValue.toLowerCase()));
        break;
      default:
        break;
    }
    ...
    <div>
      <button onClick={openModal}><svg
         ...
         <div className="cursor-pointer flex flex-col justify-center items-center gap-3 w-32 h-32  pt-4 absolute right-5 bg-white rounded-md">
                   <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="text-xs  bg-transparent w-[80%]">
