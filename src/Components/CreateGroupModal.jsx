   ...
   const handleSubGroup = ()=>{
    ...
    return(
      ...
      >
        ...
        <input type="text" placeholder='Enter group name here' className='border px-2 py-1' value={groupName}  onChange={(e) => setGroupName(e.target.value)} />
        {  
        view &&
            <input type="text" placeholder='Enter Subgroup name here ' value={subgroupName} onChange={(e)=>setSubgroupName(e.target.value)} className='border px-2 py-1 mt-2' />
        }
        <div className='flex justify-end items-center gap-5 mt-10'>
            <button className='border-primary text-black p-2 hover:text-white rounded-md text-xs hover:bg-primary' onClick={()=>handleSubGroup()}>
            Add subgroup
            </button>
        <button
          ...
          <button className="bg-primary p-2 text-white rounded-md text-xs"  onClick={()=>handleCreateGroup()}>
            Create
          </button>
        </div>
        </div>
